...
mcp = FastMCP(
    name="Habit Tracker",
    instructions="Track daily habits, log completions, and view streaks and weekly summarize!!!"
)

TEMP_DIR = tempfile.gettempdir()

# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# DATA_FILE = os.path.join(BASE_DIR, "habits.json")

DATA_FILE = os.path.join(TEMP_DIR, "habits.json")
...
def log_done(habit_name: str, log_date: str = "") -> str:
    ...
    return (
        f"Logged '{habit_name}' for {target}. "
...
def get_streaks() -> str:
    ...
    for key, habit in data["habits"].items():
        ...
        lines.append(
            f"  {status} {habit['name']}: "
            f"{streak} day streak ({total} total completions)"
        )
...
def list_habits() -> str:
    ...
    for key, habit in data["habits"].items():
        ...
        lines.append(
            f"  • {habit['name']}{desc} "
            f"(since {habit['created']}, {len(logs)} completions)"
        )
...
def weekly_summary() -> str:
    ...
    day_labels = [
        (date.today() - timedelta(days=i)).strftime("%a %d")
        for i in range(6, -1, -1)
    ]

    lines = ["Weekly summary (last 7 days):\n"]

    header = "  {:20s} ".format("Habit") + "  ".join(
        f"{d:6s}" for d in day_labels
    )
    ...
    total_done = sum(
        sum(
            1 for d in days
            if d in set(data["logs"].get(k, []))
        )
        for k in data["habits"]
    )

    percentage = (
        round(total_done / total_possible * 100)
        if total_possible
        else 0
    )
    ...
    lines.append(
        f"\n  Completion rate: "
        f"{total_done}/{total_possible} ({percentage}%)"
    )
